...
def initialize():
    return pd.read_parquet("data/st_df.gzip")
...
if len(liked_movies) == 0:
    st.header("Vous n'avez pas encore aimée des films")
    st.stop()


# ******************dataframe films recommendes ***************
liked_df = df[df["tconst"].isin(liked_movies)]

similar_ids = set(
    movie_id for sublist in liked_df["similar_films"] for movie_id in sublist
)


rec_df = df.iloc[list(similar_ids)]
rec_df = rec_df[~rec_df["tconst"].isin(liked_movies)]
...
genre_series = rec_df["Genres"].str.split(",")
...
if search:
    rec_df = rec_df[
        (rec_df["Title"].str.contains(search, case=False))
        | (rec_df["Actors"].str.contains(search, case=False))
    ]

if genre:
    for g in genre:
        rec_df = rec_df[rec_df["Genres"].str.contains(g, case=False)]

# **************Pagination setup*********************

items_per_page = 30

total_pages = (len(rec_df) - 1) // items_per_page + 1
